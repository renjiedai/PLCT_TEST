...
class QemuVM(object):
    def __init__(self,id=1,port=12055,user='root',password='openEuler12#$',vcpu=4,memory=4,
                 ...
                 bkfile='openeuler-qemu.qcow2' , path='/root/GitRepo/mugen-riscv' , gene=False, os='openEuler'):
        self.id = id
        self.port = port
        self.ip = '127.0.0.1'
        self.user = user
    ...
    def start(self):
        ...
        print(self.workingDir+self.bkFile)
        print(self.workingDir+self.drive)
        ...
        if self.os == 'openEuler':
            fw=self.workingDir+"fw_payload_oe_qemuvirt.elf"
        elif self.os =='eulaceura':
            fw=self.workingDir+"fw_boot.bin"
        ssh_port=self.port
...
if __name__ == "__main__":
    ...
    parser.add_argument('-w',type=str,default='/home/drj/Desktop/mugen/mugen-riscv/RISCV2203Testing20221029',help='Specify working directory')
    ...
    parser.add_argument('-d',type=str,default='/root/mugen/mugen-riscv',help='Specity mugen installed directory')
    parser.add_argument('-g','--generate',action='store_true',default=False,help='Generate testsuite json after running test')
    parser.add_argument('-o',type=str,default='openEuler',help='Specity the tested operation system')
    ...
    if args.list_file is not None:
        ...
        for i in range(args.x):
            qemuVM.append(QemuVM(i,ports[i],vcpu=args.c,memory=args.M,workingDir=args.w,bkfile=args.b,path=args.d.rstrip('/'),gene=args.generate,os=args.o))   
        targetQueue = Queue()
